...
CREATE TABLE `invoice` (
  ...
  `IsFinanceCharge` bit(1) DEFAULT NULL,
  ...
  `IsToBePrinted` bit(1) DEFAULT NULL,
  `IsToBeEmailed` bit(1) DEFAULT NULL,
  `IsTaxIncluded` bit(1) DEFAULT NULL,
...
CREATE TABLE `invoiceline` (
  `TxnLineID` varchar(255) NOT NULL DEFAULT '',
  ...
  KEY `fk_invoiceline_invoice1` (`IDKEY`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `Facturas`
  ADD CONSTRAINT `fk_Facturas_invoice1` FOREIGN KEY (`IDKEY`) REFERENCES `invoice` (`TxnID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
ALTER TABLE `invoiceline`
  ADD CONSTRAINT `fk_invoiceline_invoice1` FOREIGN KEY (`IDKEY`) REFERENCES `invoice` (`TxnID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
